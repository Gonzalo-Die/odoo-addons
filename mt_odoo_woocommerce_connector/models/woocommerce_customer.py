...
class Customer(models.Model):
    ...
    def create_wooc_guest_customer(self, order, instance_id):
        existing_customer = self.env['res.partner'].sudo().search([('email', '=', order["billing"].get('email'))], limit=1)

        contact_details = {}
        first = order["billing"].get('first_name')
        last = order["billing"].get('last_name')        
            
        contact_details['email'] = order["billing"].get('email')
        contact_details['phone'] = order["billing"].get('phone')
        contact_details['name'] = contact_details['email'] if (not first.strip() and not last.strip()) else first.strip() + " " + last.strip()
        ...
        if not existing_customer:
            ...
            if new_customer:              
                ...
                if order.get('billing'):
                    #billing
                    self.create_address(order.get('billing'), new_customer.id, 'invoice', 'Invoice create import ')
                
                if order.get('shipping'):
                    #shipping
                    self.create_address(order.get('shipping'), new_customer.id, 'delivery', 'Delivery create import ')
            ...
            existing_customer.sudo().write(dict_p)
            self.env.cr.commit()
