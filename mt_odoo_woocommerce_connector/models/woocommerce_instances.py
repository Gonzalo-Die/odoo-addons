...
class WooCommerceInstance(models.Model):
    ...
    wooc_all_orders_json = fields.Text(
    string='Todos los pedidos',
    compute='_compute_wooc_all_orders_json',
    store=False,
    )

    
    wooc_all_products_json = fields.Text(
        string='Todos los productos',
        compute='_compute_wooc_all_products_json',
        store=False,
    )
    ...
    def _compute_wooc_all_orders_json(self):
        """Genera un JSON con TODOS los pedidos confirmados de esta instancia,
           incluyendo fecha, estado y clase para badge."""
        for rec in self:
            # Buscar todos los sale.order de esta instancia en estado 'sale' o 'done'
            orders = self.env['sale.order'].search(
                [
                    ('woocomm_instance_id', '=', rec.id),
                    ('state', 'in', ['sale', 'done'])
                ],
            ...
            for order in orders:
                ...
                elif order.state in ('sale', 'sent', 'done'):
                    badge_class = 'badge-success'
                elif order.state in ('cancel', 'cancelled'):
                ...
                orders_data.append({
                    ...
                    'date'       : order.date_order.strftime('%d/%m/%Y %H:%M:%S') if order.date_order else '',
                    'status'     : order.state or '',
                    'badge_class': badge_class,
                })

            rec.wooc_all_orders_json = json.dumps(orders_data)



    @api.depends('sale_order_ids')  # o algún trigger relacionado, pero es suficiente recalc. cuando hay pedido
    def _compute_wooc_all_products_json(self):
        for rec in self:
            # Buscamos todos los productos importados de esta instancia.
            # Adaptar el modelo si usas product.template en lugar de product.product.
            ...
            for prod in products:
                products_data.append({
                    'id'            : prod.id,
                    'name'          : prod.name or '',
                    'qty_available' : float(prod.qty_available or 0.0),
                })
            rec.wooc_all_products_json = json.dumps(products_data)
            _logger.debug(f"JSON productos (instancia {rec.id}): {rec.wooc_all_products_json}")




    def action_import_orders(self):
        # Solo importa, sin redirigir a ninguna vista
        for rec in self:
            self.env['sale.order'].import_sale_order(rec)
            
        return True

    def action_import_and_open_orders(self):
        # Importa Y abre el tree view filtrado
        self.ensure_one()
        self.env['sale.order'].import_sale_order(self)
        action = self.env.ref('mt_odoo_woocommerce_connector.action_sale_order_tree_woocomm').read()[0]
        action.update({
            'domain': [('woocomm_instance_id', '=', self.id)],
    ...
    def connection_authenticate(self):
        ...
        if req_data.status_code == 200:
            ...
            odoo_currency = self.wooc_company_id.currency_id.name
            if currency["code"] == odoo_currency :
                _logger.info('\n\n\n\n  Currency  =  %s \n\n\n\n' % (currency["code"]) )
