...
class Product(models.Model):
    ...
    def export_product(self, wooc_instance, force_update_product=False, force_update_image=False):
        """
        Exporta cada product.template seleccionado a WooCommerce.
        En lugar de buscar el stock en stock.quant con inventory_quantity_set,
        tomamos directamente qty_available de la variante simple.
        """
        ...
        for product in products_ids:
            ...
            try:
                # Solo crear si no está ya exportado, a menos que force_update_product=True
                if not force_update_product and product.is_exported:
                    continue

                # Generamos los datos básicos del producto (sin stock aún)
                data = self.product_export_data_generate(wooc_instance, product)

                # Si el producto es “simple” (solo una variante), agregamos qty_available
                if data.get("type") == 'simple':
                    # Encontrar la variante única asociada
                    simple_variant = self.env['product.product'].sudo().search(
                        [('product_tmpl_id', '=', product.id)], limit=1)
                    # Tomar qty_available real
                    qty = simple_variant.qty_available
                    data.update({
                        "manage_stock": True,
                        "stock_quantity": qty,
                    })
                ...
                # Si ya existe en Woo (wooc_id) y está marcado como is_exported, hacemos PUT
                if product.wooc_id and product.is_exported:
                    result = woo_api.put("products/%s" % product.wooc_id, data)

                    # Si devuelve error 400/404, probamos a recrearlo (POST)
                    if result.status_code in [400, 404]:
                        ...
                        self.env['product.product'].sudo() \
                            ...
                            .write({'woocomm_variant_id': False, 'is_exported': False})
                        self.env['woocommerce.product.image'].sudo() \
                            .search([('product_template_id', '=', product.id)]) \
                            .write({'is_image_synced': False, 'is_variation_update': False, 'update_only': True})
                    ...
                    # Si no existía en Woo, lo creamos con POST y marcamos kanban
                    result = woo_api.post("products", data).json()
                    force_create_kanban = True

                if result:
                    is_product_exported = True
                    # Guardamos en Odoo los valores devueltos
                    product.wooc_id = result['id']
                    product.woocomm_regular_price = result.get('regular_price', 0.0)
                    product.woocomm_sale_price = result.get('sale_price', 0.0)
                    ...
                    product.woocomm_product_type = result.get('type', '')
                    product.default_code = result.get('sku', '') or ''

                    self.env.cr.commit()

                # Creamos/actualizamos variaciones en Woo
                self.wooc_variations_create(wooc_instance, product)

                # Si hay más variantes en Odoo que en Woo, forzamos kanban
                if len(product.product_variant_ids) > 1 and \
                   (len(product.product_variant_ids) != len(product.woocomm_variant_ids)):
                    force_create_kanban = True

                # Si toca forzar creación de kanban o no era actualización forzada
                ...
                # Si además pedimos forzar actualización de imagen, la exportamos
    ...
    def product_image_export(self, product):
        """
        Exporta las imágenes asociadas a 'product' desde Odoo a WooCommerce,
        usando la URL pública de Odoo (campo Binary wooc_image).
        """
        ...
        if w_product.status_code == 200:
            ...
            if len(image_list):
                # 1) Obtener el base_url de Odoo
                base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
                if not base_url:
                    raise UserError(_("No está configurado 'web.base.url' en los parámetros del sistema."))

                # 2) Construir la lista de imágenes para WooCommerce
                new_data = {"images": []}
                for img in image_list:
                    # Suponemos que el binario de la imagen está en el campo "wooc_image"
                    # y que Odoo expone automáticamente la URL:
                    #    /web/image/woocommerce.product.image/{img.id}/wooc_image
                    odoo_img_url = "%s/web/image/woocommerce.product.image/%s/wooc_image" % (base_url, img.id)

                    if img.is_main_image:
                        new_data['images'].insert(0, {'src': odoo_img_url})
                    else:
                        new_data['images'].append({'src': odoo_img_url})

                    # (Opcional) Marcamos que esta imagen ya la intentamos sincronizar
                    img.is_image_synced = True

                # 3) Llamar al endpoint de WooCommerce para actualizar las imágenes
                try:
                    img_update = woo_api.put("products/%s" % (product.wooc_id), new_data)
                except Exception as e:
                    _logger.error("Error conectando a WooCommerce al exportar imágenes: %s", e)
                    raise UserError(_(
                        "No se pudo conectar con WooCommerce para exportar las imágenes:\n%s"
                    ) % e)

                if img_update.status_code == 400:
                    _logger.info("400 Error al exportar imágenes: %s", img_update.json())

                if img_update.status_code == 200:
                    # 4) Si además quieres actualizar las variantes con la misma imagen:
                    image_list = self.env['woocommerce.product.image'].sudo().search([
                        ('product_template_id', '=', product.id)
                    ])
                    var_img_data = {"update": []}
                    for image in image_list:
                        for var_img in image.product_image_variant_ids:
                            var_img_data["update"].append({
                                'id': var_img.woocomm_variant_id,
                                'image': {'id': image['wooc_id']}
                            })
                    woo_api.put(
                        "products/%s/variations/batch" % (str(product.wooc_id)),
                        var_img_data
                    )
        return
